$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'merchant/mgr/settleInfo/list',
        postData : {"merchantId" : 1000},
        datatype: "json",
        colModel: [
            {label: 'ID', name: 'id', index: "id", width: 45, key: true},
            // {label: '通道地址', name: 'url', sortable: false, width: 75},
            {label: '分账支付宝userId', name: 'aliUserId', sortable: false, width: 75},
    ...
    methods: {
        ...
        updateChannel: function(value, id , merchantId){
            confirm('确定关闭或开启通道开关？', function () {
                ...
                $.ajax({
                    type: "POST",
                    url: baseURL + "merchant/mgr/updateSettleChannelFlag",
        ...
        getChannel: function(value){
            $.get(baseURL + "merchant/mgr/settleInfo/" + value, function (r) {
                if (r.list.length != 0){
        ...
        saveOrUpdate: function () {
            ...
            $.ajax({
                ...
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.merchantAliChannelList),
                success: function (r) {
                    if (r.code === 0) {
                        alert('操作成功', function () {
                            vm.reload();
        ...
        getDept: function () {
            //加载部门树
            $.get(baseURL + "merchant/mgr/selectMerchantNormal", function (r) {
